...
class RelatedPostsThumbnails {
  ...
  function get_thumbnails( $show_top = false, $posts_number_short = '', $sort_by_short = '', $main_title = '' ) {
    ...
    $query = "SELECT ID, post_content, post_excerpt, post_title,post_date FROM $wpdb->posts WHERE ID IN ('" . implode( "', '", $posts_in ) . "')";
    $posts = $wpdb->get_results( $query );
    $ttdate = array();
    foreach ( $posts as $key => $post ) {
       
        $tdate=get_post_meta($post->ID,'wpex_date');
        
        if (!empty($tdate)){
           
            $posts[$key]->post_date= date("d F Y", strtotime($tdate[0]));
            $posts[$key]->sort_key = strtotime($tdate[0]);
        }else{ 
            $posts[$key]->post_date= date("d F Y", strtotime($post->post_date));
            $posts[$key]->sort_key = strtotime($post->post_date);
        }
        $ttdate[$key] = $posts[$key]->sort_key;
    } 

  
    array_multisort($ttdate, SORT_ASC,  $posts);  
    ...
    foreach ( $posts as $post ) {
        ...
        if ( $relpoststh_output_style == 'list' ) {
         ...
         /*   if (strpos($url,'/img/default.png')===false){
            ...
            if ( $text_height != '0' ) {
                ...
                $output .= '><span class="rpth_list_content">' . $title . $excerpt . '</span>' . $after_content . '</a><p class="related-event-date" >'.$pdate.'</p><p>'.$post_excerpt.'</p></li>';
            }
          } else {
