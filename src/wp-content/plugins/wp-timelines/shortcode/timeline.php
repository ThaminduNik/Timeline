...
function parse_wpex_timeline_func($atts, $content){
	...
	global $child_idx,$parent_idx,$style,$ajax_load,$ID,$animations, $posttype,$show_media, $taxonomy,$full_content,$feature_label,$lightbox,$hide_img,$img_size,$hide_title,$back_p;
	$ajax_load = 0;
	$child_idx =0;
	$parent_idx=0;
	...
	if($the_query->have_posts()){?>
    	<div class="wpex-timeline-list <?php echo $css_class;?>" id="timeline-<?php echo esc_attr($ID);?>" data-animations="<?php echo esc_attr($animations);?>">
            ...
            <ul class="wpex wpex-timeline<?php echo $alignment!='' ? ' style-'.esc_attr($alignment) : '';?>">
				...
				if (!isset($metaData['wpex_custom_metadata']) || !in_array('milestone',$metaData['wpex_custom_metadata'])) continue;
					$i++;
				...
				if (!in_array($date_inft,$dup_date_inft)){
