...
define ("ALLOW_EMAIL_DOMAINS", serialize(array(
    'syscolabs.com',
    'mailinator.com',
    'sysco.com'
)));
if (!class_exists('wpuserAjax')) :
    class wpuserAjax
    {
            ...
            if (isset($_POST['wpuser_form_id']) && !empty($_POST['wpuser_form_id'])) {
                ...
                if (!isset($_POST['user_email'])) {
                    ...
                    if (!empty($_POST['user_email'])) {
                        ...
                        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            ...
                            if ( ! in_array($domain, unserialize(ALLOW_EMAIL_DOMAINS)))
                            {
                                $result['message'] = __('Invalid email domain', 'wpuser');
                                $result['status'] = 'warning';
                                print_r(json_encode($result));
                                exit;
                            }
                ...
                if (isset($wp_user_email) && !empty($wp_user_email)) {
                    ...
                    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                        ...
                        $parts = explode('@', $email);
                        $domain = array_pop($parts);
                 //print_r(unserialize(ALLOW_EMAIL_DOMAINS));
                        if ( ! in_array($domain, unserialize(ALLOW_EMAIL_DOMAINS)))
                        {
                            $result['message'] = __('Invalid email domain', 'wpuser');
                            $result['status'] = 'warning';
        ...
        public static function send_otp_user_email( $user_email, $intOTP, $user, $intTimeValidate = 15 ){
          ...
          $email_body_text .= '<h2 style="text-align:center">'.$intOTP.'</h2>';
                ...
                if (filter_var($wp_user_email_name, FILTER_VALIDATE_EMAIL)) {
                    ...
                    if (!empty($userInfo->ID)) {
                        ...
                        if ($wpuser_activation_key == $key) {
                            ...
                            echo '<html><head><title>Confirm your syscolabs account</title></head><body><div class="login-approved" style="text-align: center;
                            padding: 75px 0;
                            background-image: url(http://syscolabs.cleato.com/wp-content/uploads/2019/07/startup-image-01.png);
                            background-position: center center;
                            background-size: cover;
                            height: 700px;
                            background-repeat: no-repeat;"> <div class="login-approved-new" style="margin: 235px 0;
                            line-height: 2.5em;
                            font-size: 25px;
                            font-family: Arial;">';
                            _e('Your account has been approved.');
                            echo '<br/> <a class="login-approved-btn" style="background-color: #e5673e;
                            padding: 10px 45px;
                            margin-left: 10px;
        ...
        public static function wp_user_action_register_function(&$args)
        {
            ...
            if (get_option('wp_user_email_user_register_enable')) {
                ...
                $email_body_text = apply_filters('wp_user_filter_email', get_option('wp_user_email_user_register_content'), $to, $args['user_login'], null, null, null);
                $email_footer_text = 'You\'re receiving this email because you have register on ' . $site_url;
                include('template_email/template_email_defualt.php');
                $mail = wp_mail($to, $subject, $message, $headers);
            }
            ...
            if ($wp_user_default_status == 3) {
                ...
                $activate_link=admin_url('admin-ajax.php') . '?action=wpuser_activation&key=' . $random_key . '&email=' . $to;
                $email_body_text = __('Click on following link to activate your account<br><br><br>', 'wpuser');
                $email_body_text .= '<a style="    background-color: #f79468 !important;
                ...
                color: #fff;
                text-decoration: none;
                cursor: pointer;" href="'.$activate_link.'" >'.'Activate now'."</a>";
                $email_footer_text = 'You\'re receiving this email because you have register on ' . $site_url;
                include('template_email/template_email_defualt.php');
        ...
        public static function wp_user_filter_email_function($value, $userEmail = null, $userName = null, $userFirstName = null, $userLastName = null, $newPassword = null)
        {
            ...
            $replace = array(
                ...
                '{WPUSER_LOGIN_URL}' =>  get_option('siteurl'),
