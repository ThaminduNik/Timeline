...
if ( ! defined('ABSPATH')) exit;  // if direct access
	...
	foreach($layout as $item_id=>$item_info){
		...
		$item['title_link'] = '<a  class="element element_'.$item_id.' '.$item_key.' '.$custom_class.'" href="'.$item_post_permalink.'">'.apply_filters('post_grid_filter_grid_item_title',wp_trim_words(get_the_title($item_post_id), $char_limit,'')).'</a>';
		...
		$item['post_date'] = '<div class="element element_'.$item_id.' '.$item_key.' '.$custom_class.'"  >'.(!empty($time_line_date)?$time_line_date[0]:apply_filters('post_grid_filter_grid_item_post_date', get_the_date())).'</div>';
