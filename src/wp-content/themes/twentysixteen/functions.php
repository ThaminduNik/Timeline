...
add_filter( 'wptimeline_date_in_ft', 'extl_show_year_only' );
function extl_show_year_only($wpex_date ) {
   $wpex_date = get_post_meta( get_the_ID(), 'wpex_date', true );
	if(get_post_meta( get_the_ID(), 'wpex_pkdate', true )!=''&& function_exists('wpex_safe_strtotime')){
		$wpex_date = wpex_safe_strtotime(get_post_meta( get_the_ID(), 'wpex_pkdate', true ),'Y');
	}else if($wpex_date!=''){
