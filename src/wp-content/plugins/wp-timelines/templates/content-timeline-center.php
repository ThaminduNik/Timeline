<?php
global $child_idx,$parent_idx,$style, $post, $ajax_load, $ID, $animations,$posttype,$show_media, $taxonomy,$full_content,$feature_label,$lightbox,$hide_img,$img_size,$hide_title,$back_p;
...
if ($wpex_felabel==''){
    $child_idx++;
    if (($parent_idx%2==0 && $child_idx%2==1)) $class_x="index_1";
}else{
    $child_idx=0;
} 
...
?>
<li <?php post_class($class.($wpex_felabel!=''? '':' child-timeline').ex_wptl_lightbox($full_content,$ID,'class'));?> <?php echo 'data-id="filter-'.$ID.'_'.get_the_ID().'"';?> <?php echo ex_wptl_lightbox($full_content,$ID,'data'); ?>>
	<div class="<?php echo esc_attr($animations).($wpex_felabel!=''? '':"child-timeline-new ".$class_x);?>">
    ...
    <?php /*if (!empty($wpex_felabel)) { */?>
    <div class="wpex-timeline-icon <?php echo esc_attr($animations);?>">
