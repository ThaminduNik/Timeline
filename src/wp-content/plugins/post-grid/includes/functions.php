...
function post_grid_get_media($item_post_id, $media_source, $featured_img_size, $thumb_linked,$item_linked_permalink=""){
    ...
    if (!empty($item_linked_permalink))
        $thumb_custom_url =$item_linked_permalink;
    else 
    ...
    elseif($media_source == 'empty_thumb'){
        ...
        if($thumb_linked=='yes'){
            if(!empty($thumb_custom_url)){
            $html_thumb.= '<a class="custom" href="'.$thumb_custom_url.'"><img src="'.post_grid_plugin_url.'assets/frontend/css/images/placeholder.png" /></a>';
            }else{
...
function post_grid_export_content_layouts(){
	...
	if(current_user_can('manage_options')){
		...
		//$layout_content_file = fopen(post_grid_plugin_dir."/export/export-layout-content-".date('Y-m-d-h').'-'.time().".txt", "w");
...
function post_grid_meta_query_add(){
	...
	if(current_user_can('manage_options')){
		...
		?>
		<div class="items">
			<div class="header"><span class="remove"><i class="fa fa-times"></i></span>
                <span class="move pg-tooltip" title="<?php echo __('Move', 'post-grid'); ?>"><i class="fa fa-bars"></i></span>
                <span class="expand-collapse pg-tooltip" title="<?php echo __('Expand or collapse', 'post-grid'); ?>">
                <i class="fa fa-expand"></i>
                <i class="fa fa-collapse"></i>
			...
			<?php echo __('Key', 'post-grid'); ?><br />
			<input type="text" name="post_grid_meta_options[meta_query][<?php echo $key; ?>][key]" value="" /><br>
			<?php echo __('Value', 'post-grid'); ?><br />
			<input type="text" name="post_grid_meta_options[meta_query][<?php echo $key; ?>][value]" value="" /><br>
			<?php echo __('Compare', 'post-grid'); ?><br />
			<input type="text" name="post_grid_meta_options[meta_query][<?php echo $key; ?>][compare]" value="" /><br>
			<?php echo __('Type', 'post-grid'); ?><br />
			<input type="text" name="post_grid_meta_options[meta_query][<?php echo $key; ?>][type]" value="" /><br>                            
...
add_action('wp_ajax_post_grid_meta_query_add', 'post_grid_meta_query_add');
...
//        update_option('hellO_option', $layout_items_list);
		...
		<span class="remove pg-tooltip" title="'.__('Remove', 'post-grid').'"><i class="fa fa-times"></i></span>
		<span class="move pg-tooltip" title="'.__('Move', 'post-grid').'"><i class="fa fa-bars"></i></span>
		<span class="expand pg-tooltip" title="'.__('Expand or collapse', 'post-grid').'">
			<i class="fa fa-expand"></i>
    ...
    $odd_even = 0;
		...
		if($pagination_type=='load_more'){
			...
			jQuery(document).ready(function($)
				{
					$("#post-grid-'.$grid_id.' .load-more").html("'.__('No more post','post-grid').'");
					$("#post-grid-'.$grid_id.' .load-more").addClass("no-post");				
...
function post_grid_active_filter(){
	...
	if(current_user_can('manage_options')){
			...
			$html = '';
			$categories = stripslashes_deep($_POST['categories']);
			
			//var_dump($categories).'<br>';
			
			$html .= '<select class="" name="post_grid_meta_options[nav_top][active_filter]">';
			$html .= '<option  value="all">'.__('All', 'post-grid').'</option>';
